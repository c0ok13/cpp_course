...
constexpr unsigned BALL_RADIUS = 20;
constexpr unsigned REPEAT_COUNT = 10;
...
void createChecking(sf::Vector2f &mousePosition, const int WINDOW_HEIGHT, const int WINDOW_WIDTH, std::vector<ball>(&balls), PRNGColor &colorGenerator, PRNG &generator)
{
    ...
    if (Check == true)
    {
        ...
        float speedX = random(generator, 20, 150);
        float speedY = random(generator, 20, 150);
...
int main()
{
    ...
    {
        float speedX = random(generator, 70, 600);
        float speedY = random(generator, 70, 600);
    ...
    while (window.isOpen())
    {
        ...
        float partTime = dt / REPEAT_COUNT;

        for (int i = 0; i < REPEAT_COUNT; ++i)
        {
            update(balls, partTime);
            checkBall(balls);
        }
