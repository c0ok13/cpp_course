...
void update(ball (&balls)[ballsCount], float dt)
{
    ...
    {
        if ((balls[i].circle.getPosition().x + 2 * BALL_RADIUS >= WINDOW_WIDTH) && (balls[i].circle.getPosition().x > 0))
        {
            balls[i].speed.x = -balls[i].speed.x;
        }

        if ((balls[i].circle.getPosition().x < 0) && (balls[i].speed.x < 0))
        {
            balls[i].speed.x = -balls[i].speed.x;
        }

        if ((balls[i].circle.getPosition().y + 2 * BALL_RADIUS >= WINDOW_HEIGHT) && (balls[i].circle.getPosition().y > 0))
        {
            balls[i].speed.y = -balls[i].speed.y;
        }

        if ((balls[i].circle.getPosition().y < WINDOW_WIDTH) && (balls[i].circle.getPosition().y < 0))
        {
            balls[i].speed.y = -balls[i].speed.y;
        }
...
int main()
{
    ...
    sf::Vector2f position[5] = {{20.f, 80.f}, {10, 30.f}, {50.f, 5.f}, {30.f, 30.f}, {100.f, 1.f}};
