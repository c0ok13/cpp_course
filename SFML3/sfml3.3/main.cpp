...
{
    const sf::Vector2f headOffset = toEuclidean(30, 90, Eye.rotation);
...
void initEye(Eye &Eye, float posX, float posY)
{
    ...
    const sf::Vector2f PupilRadius = {10.f, 23.f};
    ...
    const sf::Vector2f ellipseRadius = {60.f, 180.f};
...
void update(const sf::Vector2f &mousePosition, Eye &Eye)
{
    ...
    const sf::Vector2f headOffset = toEuclidean(30, 90, Eye.rotation);
    Eye.pupil.setPosition(Eye.position + headOffset);
    if (std::hypot(delta.x, delta.y) <= std::hypot(headOffset.x, headOffset.y))
    {
